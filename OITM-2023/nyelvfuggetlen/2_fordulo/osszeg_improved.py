...
def find_continous_sums(numbers: np.ndarray) -> dict:
    prefix_sum = np.cumsum(np.insert(numbers, 0, 0))  # Compute prefix sum array
    sums = {}
    sum_dict = {0: -1}  # Initialize hash map
    for i, total in enumerate(prefix_sum[1:], 1):
        if total in sum_dict and i - sum_dict[total] > 1:
            sums[(sum_dict[total] + 1, i - 1)] = i
